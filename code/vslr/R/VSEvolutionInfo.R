...
EvolutionInfo <- R6Class("EvolutionInfo",
  public = list(
    ...
    #' @param filename name of the data set employed
    ...
    generateBoundsFile = function(filename, maxIter, nParentSets,
                                  bnlearnTrain, bnlearnTest, mapTest,
                                  finalbma, finalbay){
      # compose the name of the file with the data
      cat("filename with data: ", filename, "\n")
      basename <- gsub(pattern = ".csv", "", filename)
      position <- stringi::stri_locate_last_fixed(basename, "/")[,1]
      basename <- substring(basename, position+1,)
      basename <- paste(basename, maxIter, sep = "-")
      basename <- paste(basename, nParentSets, sep = "-")
      basename <- paste0(basename, ".evol")
    ...
    generateMapsGraphic = function(iteration,
                                     ...
                                     bnlearnTest = -Inf,
                                     mapTest = -Inf){
      cat("generate bounds graphic: ", "iteration: ", iteration, " bn train: ", bnlearnTrain,
          " bn test: ", bnlearnTest, " finalTest: ", mapTest, "\n")
      # compose the name for the graph
      filename <- paste(private$folder, "mapEvolution-", sep="/")
      filename <- paste0(filename, iteration)
      filename <- paste0(filename, ".pdf")
      cat("file to generate: ", filename)
    ...
    generateBMAsGraphic = function(iteration,
                                        ...
                                        mapTest = -Inf){
      # compose the name for the graph
      filename <- paste(private$folder, "bmaEvolution-", sep="/")
      filename <- paste0(filename, iteration)
      filename <- paste0(filename, ".pdf")
    ...
    generateBAYsGraphic = function(iteration,
                                        ...
                                        mapTest = -Inf){
      # compose the name for the graph
      filename <- paste(private$folder, "bayEvolution-", sep="/")
      filename <- paste0(filename, iteration)
      filename <- paste0(filename, ".pdf")
    ...
    generateEntropyGraphic = function(){
      # compose the name for the graph
      filename <- paste(private$folder, "entropyEvolution.pdf", sep="/")
