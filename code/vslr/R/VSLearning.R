...
VSLearning <- R6Class("VSLearning",
  public = list(
    ...
    learn = function() {
      ...
      # return the measures
      c(private$finalMapTrain, private$finalBmaTrain, private$finalBayTrain,
        private$finalMapTest, private$finalBmaTest, private$finalBayTest)
    },
  ...
  updateBounds = function(entropy, map, bma, bay){
    ...
    cat("new bound: ", private$currentMAP, " (", map, " + ", entropy, ")\n")
	  ...
	  for(i in 1:dimr[1]){
	    for(j in 1:dimr[2]){
          ...
          for(i in 1:nr){
             ...
             ones <- sum(row)
             if(ones > 10){
                result[i,] <- private$limitOnes(row, 10) 
